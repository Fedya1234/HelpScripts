...
public class DecalPainter : MonoBehaviour {
   ...
   public void MakeStep(float intVar = 0f)
    {
        StartCoroutine(Paint(transform.position + transform.right* intVar*0.25f, stepColor, 1));
    }

    public IEnumerator Paint (Vector3 location, Color color, int drops, float scaleBonus = 1f) {
        location += Vector3.up;
#if UNITY_EDITOR
        mHitPoint = location;
        mRaysDebug.Clear ();
        ...
        var dir = transform.TransformDirection (-Vector3.up * SplashRange);

        // Avoid raycast backward as we're in a 2D space
        if (dir.z < 0) dir.z = Random.Range (0f, 0f);

        // Raycast around the position to splash everwhere we can

        if (Physics.Raycast (location, dir, out hit, 20, layer)) {
    ...
    private void PaintDecal (RaycastHit hit, Color color, float scaleBonus) {
        //     Debug.LogError ("Paint"); // Create a splash if we found a surface
        ...
        //        Debug.LogError (paintSplatter.name);
        // Find an existing material to enable batching
        var sharedMat = materials.Where (m => m.name.Equals (paintSplatter.GetComponentInChildren<Renderer> ().material.name) &&
            m.color.Equals (color)
        ).FirstOrDefault ();

        // New material
        if (sharedMat == null) {
            Material mat = paintSplatter.GetComponentInChildren<Renderer> ().material;
            mat.color = color;
        ...
        // Old one
            ...
            paintSplatter.GetComponentInChildren<Renderer> ().sharedMaterial = sharedMat;
        }
