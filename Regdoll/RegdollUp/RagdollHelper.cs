...
public class RagdollHelper : MonoBehaviour {
		...
		set{
			if (value==true){
				...
				if (state==RagdollState.ragdolled) {
                    ...
                    //Debug.Log(ragdolledFeetPosition.ToString());
                    //Debug.Log(anim.ToString());
                    //Debug.Log(anim.GetBoneTransform(HumanBodyBones.LeftFoot));
					...
					{
						anim.SetTrigger("UpFront");
					}
					else{
						anim.SetTrigger("UpBack");
					}
				} //if (state==RagdollState.ragdolled)
			}	//if value==false	
		} //set
	...
	void setKinematic(bool newValue)
	{
        ...
        {
            _mainCollider.enabled = newValue;
        }
	...
	void LateUpdate()
	{
		...
		if (state==RagdollState.blendToAnim)
		{
			if (Time.time<=ragdollingEndTime+mecanimToGetUpTransitionTime)
			{
				...
				Vector3 newRootPosition=transform.parent.position + animatedToRagdolled;
					
				//Now cast a ray from the computed position downwards and find the highest hit that does not belong to the character 
				RaycastHit[] hits=Physics.RaycastAll(new Ray(newRootPosition+Vector3.up*1f,Vector3.down)); 
				newRootPosition.y=0;
				...
				transform.parent.position=newRootPosition;
				...
				transform.parent.rotation*=Quaternion.FromToRotation(animatedDirection.normalized,ragdolledDirection.normalized);
			}
